...
const PRACTICE_TYPES = ["mcq", "true-false", "fill-blank"];
function normalisePracticeQuestion(question = {}, id = "q") {
  const type = PRACTICE_TYPES.includes(question.type) ? question.type : "mcq";
  const rawOptions = Array.isArray(question.options) ? question.options : [];
  const options = type === "mcq"
    ? rawOptions.map((item) => String(item ?? "").trim().slice(0, 300)).slice(0, 4)
    : ["True", "False"];
  const answers = type === "fill-blank"
    ? [...new Set((Array.isArray(question.answers) ? question.answers : String(question.answers || "").split(",")).map((item) => String(item).trim()).filter(Boolean))]
    : [];
  return {
    id: String(question.id || id),
    type,
    question: String(question.question || "").trim().slice(0, 1200),
    points: Math.max(1, Math.min(20, Number(question.points) || 1)),
    options,
    correctIndex: type === "fill-blank" ? 0 : Math.max(0, Math.min(type === "true-false" ? 1 : 3, Number(question.correctIndex) || 0)),
    answers,
  };
}
function normalisePracticeRecord(id, raw = {}) {
  const classNumber = Number(raw.class);
  const questionsNode = raw.questions && typeof raw.questions === "object" ? raw.questions : {};
  const questions = Object.entries(questionsNode).map(([qid, value]) => normalisePracticeQuestion(value, qid));
  return {
    id: String(raw.id || id), title: String(raw.title || "Practice Test").trim(), class: CLASSES.includes(classNumber) ? classNumber : null,
    subject: String(raw.subject || "").toLowerCase(), durationSeconds: Math.max(60, Math.min(10800, Number(raw.durationSeconds) || 900)),
    active: raw.active === true, createdAt: Number(raw.createdAt) || 0, updatedAt: Number(raw.updatedAt) || Number(raw.createdAt) || 0, questions,
  };
}
function validatePracticeInput(input) {
  const classNumber = Number(input.class), subject = String(input.subject || "").toLowerCase();
  if (!CLASSES.includes(classNumber)) throw new Error("INVALID_CLASS");
  if (!SUBJECTS.some((item) => item.id === subject)) throw new Error("INVALID_SUBJECT");
  if (String(input.title || "").trim().length < 2) throw new Error("INVALID_PRACTICE_TITLE");
  const duration = Number(input.durationSeconds);
  if (!Number.isFinite(duration) || duration < 60 || duration > 10800) throw new Error("INVALID_PRACTICE_DURATION");
  if (!Array.isArray(input.questions) || input.questions.length < 1 || input.questions.length > 50) throw new Error("INVALID_PRACTICE_QUESTIONS");
  const questions = input.questions.map((question, index) => {
    const q = normalisePracticeQuestion(question, `q-${index + 1}`);
    if (q.question.length < 2) throw new Error("INVALID_PRACTICE_QUESTION");
    if (q.type === "mcq" && q.options.filter(Boolean).length < 2) throw new Error("INVALID_PRACTICE_OPTIONS");
    if (q.type === "mcq" && q.correctIndex >= q.options.length) throw new Error("INVALID_PRACTICE_ANSWER");
    if (q.type === "fill-blank" && !q.answers.length) throw new Error("INVALID_PRACTICE_ANSWER");
    return q;
  });
  return { classNumber, subject, title: String(input.title).trim().slice(0, 160), durationSeconds: Math.round(duration), questions };
}
function practiceRecordObject(input, active, id, createdAt) {
  const checked = validatePracticeInput(input);
  const questions = {};
  checked.questions.forEach((question, index) => {
    const qid = String(question.id || `q-${index + 1}`).replace(/[^A-Za-z0-9_-]/g, "-").slice(0, 80) || `q-${index + 1}`;
    questions[qid] = { ...question, id: qid };
  });
  const now = Date.now();
  return {
    id: String(id), title: checked.title, class: checked.classNumber, subject: checked.subject,
    durationSeconds: checked.durationSeconds, active: Boolean(active), questions,
    createdAt: Number(createdAt) || now, updatedAt: now,
  };
}
export async function loadAllPractice() {
  const snapshot = await withTimeout(get(ref(database, "practiceCatalog")), 15000);
  const root = snapshot.val() || {};
  const practice = [];
  Object.entries(root).forEach(([classKey, subjectNode]) => Object.entries(subjectNode || {}).forEach(([id, raw]) => {
    const item = normalisePracticeRecord(id, raw);
    if (item.class) practice.push(item);
  }));
  practice.sort((a, b) => (b.updatedAt - a.updatedAt) || a.title.localeCompare(b.title, undefined, { sensitivity: "base" }));
  return practice;
}
export async function savePracticeSet({ input, publish = true, existing = null }) {
  const id = existing?.id || `p-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const record = practiceRecordObject(input, publish, id, existing?.createdAt);
  const updates = { [`practiceCatalog/class-${record.class}/${record.id}`]: record };
  if (publish) updates[`publishedPractice/class-${record.class}/${record.subject}/${record.id}`] = record;
  else if (existing) updates[`publishedPractice/class-${record.class}/${record.subject}/${record.id}`] = null;
  if (existing && (existing.class !== record.class || existing.subject !== record.subject)) {
    updates[`practiceCatalog/class-${existing.class}/${existing.id}`] = null;
    updates[`publishedPractice/class-${existing.class}/${existing.subject}/${existing.id}`] = null;
  }
  await withTimeout(update(ref(database), updates), 15000);
  return record;
}
export async function publishPracticeSet(practice, publish) {
  if (!practice?.id) throw new Error("PRACTICE_NOT_FOUND");
  const record = { ...practice, active: Boolean(publish), updatedAt: Date.now() };
  const updates = { [`practiceCatalog/class-${practice.class}/${practice.id}`]: record, [`publishedPractice/class-${practice.class}/${practice.subject}/${practice.id}`]: publish ? record : null };
  await withTimeout(update(ref(database), updates), 15000);
  return record;
}
export async function deletePracticeSet(practice) {
  if (!practice?.id) throw new Error("PRACTICE_NOT_FOUND");
  await withTimeout(update(ref(database), {
    [`practiceCatalog/class-${practice.class}/${practice.id}`]: null,
    [`publishedPractice/class-${practice.class}/${practice.subject}/${practice.id}`]: null,
  }), 15000);
}
